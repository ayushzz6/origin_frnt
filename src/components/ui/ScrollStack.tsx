...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback((passedScrollTop?: number) => {
    if (!cardsRef.current.length || isUpdatingRef.current) return;

    isUpdatingRef.current = true;

    const { scrollTop: currentScrollTop, containerHeight, scrollContainer } = getScrollData();
  ...
  const setupScrollListener = useCallback(() => {
    ...
    if (useWindowScroll) {
      ...
      return () => scroller.removeEventListener('scroll', handleScrollEvent);
    }
