...
const Prism: React.FC<PrismProps> = ({
  ...
  useEffect(() => {
    ...
    const fragment = /* glsl */ `
      ...
      void main(){
        ...
        for (int i = 0; i < STEPS; i++) {
          ...
          vec3 q = p;
          q.y += centerShift;
          // SHARPNESS TWEAK: Tighter falloff for sharper edges
          // Original: d = 0.1 + 0.2 * abs(sdPyramidUpInv(q));
    ...
    const onMove = (e: PointerEvent) => {
      const ww = Math.max(1, winSize.w);
      const wh = Math.max(1, winSize.h);
      const cx = ww * 0.5;
      const cy = wh * 0.5;
    ...
    const render = (t: number) => {
      ...
      if (animationType === 'hover') {
        ...
        // Combine mouse interaction with scroll rotation
        ...
        yaw = tScaled * wY; // Also apply to 3drotate
        pitch = Math.sin(tScaled * wX + phX) * 0.6;
        roll = Math.sin(tScaled * wZ + phZ) * 0.5;
        program.uniforms.uRot.value = setMat3FromEuler(yaw, pitch, roll, rotBuf);
        if (TS < 1e-6) continueRAF = false;
      } else {
        // Base case with potential scroll
