/**
 * Idempotent runtime ensure for Phase 5 assessment schema.
 * Canonical SQL: src/db/migrations/20260520_phase5_teacher_tests.sql
 */
...
declare global {
  var __originAssessmentSchemaEnsured: boolean | undefined;
  var __originAssessmentSchemaPromise: Promise<void> | undefined;
}

const MIGRATION_ID = "20260520_phase5_teacher_tests";
...
async function recordMigration(client: PoolClient): Promise<void> {
  await client.query(
    ...
    [MIGRATION_ID, "phase 5 teacher tests"],
  );
}

export async function ensureAssessmentSchema(): Promise<void> {
  if (!isUserPostgresConfigured()) return;
  if (globalThis.__originAssessmentSchemaEnsured) return;
  if (!globalThis.__originAssessmentSchemaPromise) {
    globalThis.__originAssessmentSchemaPromise = (async () => {
      ...
      try {
        ...
        globalThis.__originAssessmentSchemaEnsured = true;
      } catch (error) {
      ...
      globalThis.__originAssessmentSchemaPromise = undefined;
      throw error;
    });
  }
  await globalThis.__originAssessmentSchemaPromise;
}